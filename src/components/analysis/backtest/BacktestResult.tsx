...
import { BacktestReliabilityPanel } from '@/components/analysis/BacktestReliabilityPanel'
import type { TradeData } from '@/lib/backtest/reliability-scorer'
import type { EquityPoint as LibEquityPoint } from '@/lib/backtest/risk-metrics'
...
export function BacktestResult({
  ...
  const convertTrades = (trades: BacktestTrade[]): TradeData[] => {
    return trades.map(t => ({
      symbol: t.symbol,
  ...
  const convertEquityCurve = (curve: EquityPoint[]): LibEquityPoint[] => {
    return curve.map(p => ({
    ...
    <>
      ...
      {selectedPlan && (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            <TabsTrigger value="overview">回测结果</TabsTrigger>
            <TabsTrigger value="reliability">可靠性评估</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="reliability">
            <BacktestReliabilityPanel
              trades={convertTrades(trades)}
              equityCurve={convertEquityCurve(equityCurve)}
            />
          </TabsContent>
        </Tabs>
      )}
